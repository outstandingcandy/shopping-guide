...
class SmzdmPipeline(object):
    ...
    def __init__(self):
        ...
        # Create table
        self.c.execute('''CREATE TABLE IF NOT EXISTS %s (url text unique, title text, description text, price float, \
                                                        worthy_vote int, unworthy_vote int, favorite_count int, comment_count int, \
                                                        referer text, img_md5_list text)''' % (self.webpage_database_name))
        self.conn.commit()
